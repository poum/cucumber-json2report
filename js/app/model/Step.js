...
Ext.define('CJ2H.model.Step', {
  ...
  attach: function(parentNode) {
    ...
    var stepNode = { expanded: true, iconCls: 'window_' + this.get('status'), leaf: false };
    stepNode = parentNode.appendChild(stepNode);

    stepNode.set('text', this.get('keyword') + ' ' + this.get('name'));
    stepNode.set('duration', this.get('duration'));
    stepNode.set('status', this.get('status'));
    stepNode.set('stepId', this.get('id'));

    var embeddings = Ext.getStore('Embeddings');
    embeddings.filter([{ property: 'step', value: this.get('id'), exactMatch: true }]);

    ecpt = 1;
    embeddings.each(function(embed) {
      embed.attach(stepNode, ecpt + '/' + embeddings.count());
      ecpt++;
    });
    embeddings.clearFilter();
  }
